...
```{r, out.width='75%', fig.align='center', fig.cap="Boxplot of points scored by event."}
...
Events that involve throwing such as Javelin, Pole vault, Shot put and Discus tend to yield a lower points score. Meanwhile track events such as 400 metres, 100 metres, and 110 metre hurdles are where the decathletes collect the most points. This may suggest decathletes are more likely to compete with olympians in a running event than a throwing event.

In the [Decathletes by Continent] section we'll investigate if there is a significant difference between decathletes from different continents. The table below shows the average total points by continent, as well as standard deviation and sample size. 
...
We'll start with the 100m sprint, testing if decathletes are significantly slower than Olympic sprinters. To do this we'll use a two-sample t-test. This compares the mean of two groups and tests to see if they are significantly different from each other. The table below shows the average 100m time for the decathletes and the olympians. It's clear the olympians yield a better result, they average a time that's over two seconds faster than the decathletes, but is this difference statistically significant?

```{r}
hundred_m <- events_to_compare('100M Men', '100m', 'greater')
knitr::kable(hundred_m[[2]] %>% mutate(athlete_type = ifelse(athlete_type == "decathlete", "Decathlete", "Olympian")) %>% rename(`Athlete Type` = athlete_type, Mean = mean, `Std Dev` = sd, `Sample Size` = n), digits = 3, caption = "Summary of 100m sprint time for decathletes and olympians.")
```
...
Let's check these two assumptions hold true for our data. To check for normality, a simple Q-Q (Quantile-Quantile) plot can be used. Below is the Q-Q plot for the decathletes. The second Q-Q plot, for the olympians, can be found in [Appendix B].
...
The resulting p-value is far smaller than our significance level (0.05). This means we can strongly reject $H_0$ in favour of $H_1$ and conclude that the decathletes are significantly slower at running the 100m than the Olympic sprinters. The 95% confidence interval suggests the true difference in 100m time between decathletes and Olympic sprinters is more than two seconds. This probably isn't a surprising result, winning an Olympic medal in the 100m sprint is extremely difficult and we'd expect no other cohort to compare. 
...
knitr::kable(discus[[2]] %>% 
               ...
               rename(Discus = athlete_type,
                      ...
                      `Std Dev` = sd,
                      `Sample Size` = n),
             caption = "Summary of distance decathletes and olympians throw the discus.")
```
...
knitr::kable(high_jump[[2]] %>% 
               ...
               rename(`High Jump` = athlete_type,
                      ...
                      `Std Dev` = sd,
                      `Sample Size` = n),
             caption = "Summary of height decathletes and olympians reach on the high jump.")
```
